      ...
      <View style={{ flex: 1 }}>
        ...
        {visible && (
          <View style={StyleSheet.absoluteFill} pointerEvents="box-none">
            {/* خلفية معتمة */}
            <Animated.View style={[styles.backdrop, { opacity: anim.interpolate({ inputRange: [0, 1], outputRange: [0, 0.65] }) }]}>
              <TouchableOpacity style={{ flex: 1 }} activeOpacity={1} onPress={close} />
            </Animated.View>

            {/* اللوحة */}
            <Animated.View style={[styles.panel, {
              transform: [{ translateX: anim.interpolate({ inputRange: [0, 1], outputRange: [MENU_W, 0] }) }],
            }]}>
              {/* الرأس */}
              <View style={styles.headerRow}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                  <Image source={require('../../assets/icon.png')} style={styles.logo} />
                  <Text style={styles.logoText}>Rabah <Text style={{ color: colors.purple }}>Tec</Text></Text>
                </View>
              ...
              {/* العناصر */}
              {ITEMS.map((item, i) => (
                <TouchableOpacity key={item.label} style={[styles.item, i === 0 && styles.itemActive]} onPress={() => go(item)}>
                  <Text style={[styles.itemLabel, i === 0 && { color: colors.purple }]}>{item.label}</Text>
                  <Ionicons name={item.icon} size={20} color={i === 0 ? colors.purple : colors.textDim} />
                </TouchableOpacity>
              ))}
              ...
              <TouchableOpacity style={styles.item} onPress={logout}>
                <Text style={[styles.itemLabel, { color: '#EF4444' }]}>تسجيل الخروج</Text>
...
const styles = StyleSheet.create({
  ...
  panel: {
    position: 'absolute', top: 0, bottom: 0, right: 0, width: MENU_W,
    backgroundColor: colors.card, borderLeftWidth: 1, borderLeftColor: colors.border,
    paddingTop: 54, paddingHorizontal: 18, paddingBottom: 24,
  },
  headerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 },
  logo: { width: 34, height: 34, borderRadius: 9 },
  logoText: { color: colors.text, fontSize: 18, fontWeight: 'bold' },
  item: { flexDirection: 'row', justifyContent: 'flex-end', alignItems: 'center', gap: 12, paddingVertical: 14, paddingHorizontal: 10, borderRadius: 12 },
